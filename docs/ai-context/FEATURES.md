...
`LiveMapScreen` distinguishes manual pause from temporary story pause. `demoWalk.tick` re-emits the exact last position during pause, including after an explicit place selection. Guide me requests a pedestrian route over mapped streets and paths, draws it as a solid blue line, follows the visitor and reroutes after roughly 20 metres of movement. Navigation stays inside Sākṣī and offers an in-app retry if routing fails; it never redirects to Google Maps. Routing uses the public OpenStreetMap Valhalla endpoint by default or `EXPO_PUBLIC_ROUTING_URL` for a deployment-owned instance.
