 ...
 *
 * Generates realistic observation & condition-report history. Exports
 * `generateHistory()` so other tools (the mock API's file, and
 * tools/seed-history-supabase.mjs for a staging Supabase project) share one
 * generator instead of drifting apart. Run directly, it writes 30 days of
 * that history to seed/history.json for the mock-api server to load, so the
 * dashboard and coverage metrics look lived-in from first boot.
 *
 * Usage:
...
/**
 * Builds `days` of synthetic captures, condition reports, merit events and
 * acknowledgements against the given sites/vantages. `corroborations` and
 * `status: 'corroborated'` on a report are mock-schema concepts with no
 * production column (see docs/PRODUCTION-PILOT.md) — a consumer targeting
 * the real Supabase schema should treat 'corroborated' as still 'open'.
 */
export function generateHistory({ days = 30, sites, vantages } = {}) {
  const siteList = sites ?? JSON.parse(readFileSync(join(ROOT, 'seed/sites.json'), 'utf8'));
  const vantageList = vantages ?? JSON.parse(readFileSync(join(ROOT, 'seed/vantages.json'), 'utf8'));
  const activeVantages = vantageList.filter((v) => v.active);
  ...
  return { captures, reports, meritEvents, acknowledgements, sites: siteList, vantages: vantageList };
}

// --- CLI entry point ---------------------------------------------------------
// Only runs when this file is executed directly, not when generateHistory is
// imported by another tool.
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const { captures, reports, meritEvents, acknowledgements, sites, vantages } = generateHistory({ days: DAYS });
