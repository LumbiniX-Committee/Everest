...
*Prepared 2026-08-31. Every figure below is verifiable against this repository
(`LumbiniX-Committee/Everest`) at commit `02f6933`. Fields marked
**[FILL IN]** need information only the team has.*
...
```
> Nothing is publicly deployed yet. The GitHub repository is the honest answer
> here. If you deploy the Next.js landing page (in `landing/`) before the
> deadline — Vercel would take about ten minutes — replace this with the live
> URL, as a working public page reads considerably stronger than a repo link.
...
A working prototype, in the field-testable sense rather than the slideware
sense. It won first place at the LumbiniX 2026 national hackathon, and has since
been substantially expanded from a single-site demonstration into a
multi-region platform. Concretely, as of this submission:
...
- A citation-locked question-answering engine scoring 68/68 on its own
  adversarial benchmark, with zero fabricated citations.
- A custodian dashboard, delivered both as a web application and as an in-app
  screen, closing the loop from a visitor's report to a caretaker's
  acknowledgement.
- 127 automated tests, plus an automated content and vocabulary gate, all
  passing.
- Installable Android builds produced through EAS.

The application runs on Android and iOS via Expo, with a public GitHub
repository. It is not yet deployed as a public web service, and no institutional
agreement has been signed — both are stated plainly in the project's own
...
This is measured, not asserted. The engine scores 68/68 on a purpose-built
benchmark spanning answerable questions, deliberately adjacent questions,
out-of-scope questions that must be refused, adversarial impersonation and
prompt-injection attempts, Nepali-language questions, and heritage-conservation
questions — with the count of "citations naming an unretrieved passage" held at
zero. The benchmark runs in the automated verification gate, so a regression
breaks the build.
...
Data exports as CSV and GeoJSON with real per-report coordinates, opening
directly in QGIS as a working layer rather than as a screenshot of one.
...
A single command runs a type check, 127 unit tests, a content-integrity
validator, a vocabulary linter, and the citation benchmark. The vocabulary
linter mechanically rejects gamification language ("points", "streak",
"leaderboard", "rewards") from visitor-facing copy, because the project refuses
to gamify a sacred site. These properties are structural, and a contributor
cannot quietly erode them.

8. NEPALI-FIRST, AND A PUBLISHED ETHICS POLICY

Content is bilingual Nepali and English, with source text and citations never
machine-translated. The project publishes a policy refusing money from any
...
a way to act on each one visibly. For a resource-constrained institution, an
evidence stream that costs nothing to collect changes what is possible.
...
systems fabricate confidently. Sākṣī demonstrates, in shipped code with a
published benchmark, that an AI feature can be built to refuse rather than
invent, and that this property can be enforced by automated tests rather than
promised in a policy document. That pattern transfers directly to health,
legal, agricultural and government applications where a plausible fabrication
causes real harm. A Nepali student team demonstrating this is a signal about
where the country's engineering capability actually sits.
...
site it monitors. It will not dress a reconstruction as a photograph. These
refusals cost money by design, and they are the reason a government body could
...
```
IT IS BUILT, NOT PROPOSED

The clearest evidence of effective implementation is that the system works
end-to-end today and its claims are measurable:

- 68/68 on the citation benchmark, with zero fabricated citations, run
  automatically on every change.
- mAP50 0.8167 for the on-device damage detector, displayed in the app itself
  and never rounded up.
- 127 automated tests passing, alongside a type check, a seed-data integrity
  validator and a vocabulary linter, in one command.
- 15 sites across 2 regions, 12 surveyed vantages, 18 Patan monument records and
  40 Kathmandu Valley landmark records.
- Installable Android builds shipping through EAS.

THE FULL LOOP HAS BEEN DEMONSTRATED
...
with a note. That entire loop has been exercised end to end against a running
API, in a browser, with the resulting state change verified — not storyboarded.
...
the interface. Map tiles come from an open global source; the retrieval index is
...
area, were added on the existing architecture without a rewrite.

VALUE DELIVERED TO EACH SIDE

To the custodian: an evidence stream they did not fund, in formats their
existing tools already read. The GeoJSON and CSV exports carry real per-report
coordinates and open as a working QGIS layer.
...
No institutional agreement has been signed; outreach is under way. Some site
coordinates remain documentary approximations rather than survey-grade, and are
labelled as such in the interface. The damage detector covers cracks only. Three
recently added sites do not yet carry historical reconstruction plates. These
limits are published in the project's own README and status documents, because a
monitoring system that overstates its own reliability is worse than one that is
honestly incomplete.
```
...
2. RETRIEVAL-AUGMENTED GENERATION WITH ENFORCED CITATION VALIDATION
...
The distinguishing component is post-generation citation validation: the engine
parses every bracketed citation out of its own answer and discards the entire
answer if any citation does not resolve to a passage that was actually
retrieved. Grounding is therefore verified, not merely encouraged by a prompt.
The corpus combines the Pali canon (from the open bilara-data corpus) with a
hand-verified heritage corpus: verbatim articles from the ICOMOS Venice Charter
(1964) and Burra Charter (2013), UNESCO World Heritage inscription records for
Lumbini and the Kathmandu Valley, and named Kathmandu Valley archaeology.
...
4. CLOUD LLM SYNTHESIS (optional, server-mediated)

For richer prose the system can call a hosted model (Ollama Cloud). The API key
is held server-side and is never bundled into the mobile application. A fast-fail
circuit breaker probes reachability and falls back to the deterministic on-device
engine after a single short timeout, so an unreachable endpoint degrades the
answer's style without ever degrading its correctness or availability.
...
row-level security) treated strictly as a copy. The security posture is
deliberately asymmetric: the anonymous client role may insert and update
observations but may never select them, so a leaked publishable key can add to
the record but cannot harvest it.

8. CROSS-PLATFORM AND TOOLING

React Native 0.86 on Expo SDK 57, TypeScript 6 in strict mode, Next.js 16 for
the custodian web dashboard, and EAS for cloud builds.
```
...
This was tested rather than asserted. After the hackathon, three Kathmandu
Valley monument-zone sites roughly 200 km from the original pilot area were
added on the same codebase, with a region concept introduced in a single change
and no rewrite of the map, geofencing, capture or dashboard systems.
...
run one full monitoring cycle with real custodian usage. Extend vantage coverage
across all seven Kathmandu Valley monument zones. Establish the dhunge dhara
network as the flagship monitored class, because it is numerous, measurably
declining, largely undocumented, and its condition (flowing or dry) is legible in
a single photograph. The success metric is not downloads; it is the number of
vantages with more than one dated observation, and the median time an institution
takes to acknowledge a report.
...
to the other common masonry pathologies. Introduce institutional accounts with
proper authentication, replacing the deliberately simple custodian identity used
in the prototype.
...
the mechanism is specific to Nepal or to Buddhist heritage; the citation engine
already answers on international conservation doctrine, and the site model is
region-parameterised. Growth here is partnership-led rather than consumer-led:
one heritage authority at a time, each bringing its own visitor flow.
...
rely on. Our question-answering engine validates its own citations and discards
any answer containing a reference it cannot resolve; it scores 68/68 on an
adversarial benchmark including impersonation and prompt-injection attempts,
with fabricated citations held at zero. Our damage detector publishes its real
accuracy, 0.8167, inside the app, and marks its findings as candidates for a
human to confirm rather than verdicts. When the app cannot measure something, it
records "unknown" rather than a convenient zero.
...
winning demo, we widened it into a platform — extending the AI corpus to
international conservation doctrine, adding three Kathmandu Valley UNESCO sites
roughly 200 km outside the original area, and building the custodian dashboard
that turns collected evidence into institutional action. The system today is
materially more capable than the one that won.
...
entirely. We label every reconstruction so it can never be mistaken for a
photograph. Each of these decisions costs us something, and together they are
the reason a government institution could plausibly trust the record we produce.

WE STATE OUR LIMITATIONS IN PUBLIC

Our own documentation lists what is unfinished: no signed institutional
agreement yet, some coordinates still documentary rather than survey-grade, a
detector that covers cracks only, three sites without historical plates. We
believe a team willing to publish its own gaps is exactly the kind of team this
award should be encouraging, and it is the same discipline that makes the
underlying evidence worth having.
...
| 2 | Presentation (PDF or PPT) | **Ready to build** | Build from `slides.md` in this repo — it is a complete 18-slide plan with talking points and a screenshot checklist. Export to PDF. |
| 3 | Other supporting documents | **Partly ready** | Strong options: `POST-PIVOT-STATUS.md` (engineering status, limitations stated), the Dhamma benchmark output (68/68), and the hackathon certificate. |
| 4 | Video (up to 3 minutes) | **Not recorded** | Script is ready in `video-demo.md`. See the note below — this is your highest-value remaining task. |
...
| 6 | Demo access / credentials | **N/A** | See suggested text below. |
| 7 | Relevant web links **(required)** | **Ready** | See below. |

### Suggested text — Demo Access or Credentials
```
No login or credentials are required. Sākṣī is an offline-first mobile
application with no account system by design: every record is written to the
device first, and the app is fully usable with no sign-in and no network.

Two ways to evaluate it:

1. Android APK — an installable build can be provided on request, or produced
   from source with `npx eas build --profile preview --platform android`.

2. From source — the repository runs with:
   ...
   npm run verify     (type check, 127 tests, content validation, vocabulary
                       lint, and the 68/68 citation benchmark)

The custodian dashboard runs locally with `npm run dev` inside the landing/
directory, at http://localhost:3000/custodian.

[FILL IN — if you publish an APK link or deploy the web dashboard before
judging, add the public URLs here.]
```
...
- Deployed project website / landing page URL
- APK download link
...
> asset you are missing. `video-demo.md` in the repo gives you a shot-by-shot
> script with timings. If you record only one thing, record the closed loop:
> file a condition report on the phone, then cut to the custodian dashboard and
> acknowledge it. That sequence is what separates this from a concept.
...
1. Record the 3-minute video (`video-demo.md`)
2. Deploy the landing page publicly — roughly ten minutes on Vercel — so
   "Official Website" is a live page rather than a repository
3. Export `slides.md` to a PDF presentation
4. Attach hackathon certificate or third-party proof of the first-place result
...
|---|---|---|
| 68/68 citation benchmark, 0 fabricated | `core/dhamma/eval.ts` | `npm run eval:dhamma` |
| 127 automated tests | `tools/run-tests.mjs` | `npm run test` |
...
| Expo SDK 57, RN 0.86.2, TS 6 | `package.json` | `cat package.json` |
